...
if __name__ == "__main__":
    ...
    print("Load autoencoder model from {}".format(
        cf.FINAL_MODEL_PATH.format(name + '/dfencoder_{}_{}.pt'.format(version, emb_size))))
    ...
    for index_ in list_[:n_sample]:
        ...
        df_counterfactual.to_csv(cf.FINAL_RESULT_PATH.format(
            "adult/" + mode + "/mobj-ninstance-{}-nsample-{}-size-{}.csv".format(n_instance, n_sample, emb_size)),
                                 index=False)
        # break

    print("Output file to ", cf.FINAL_RESULT_PATH.format(
        "adult/" + mode + "/mobj-ninstance-{}-nsample-{}-size-{}.csv".format(n_instance, n_sample, emb_size)))
    ...
    #                          index = False)    
    df_counterfactual.to_csv(cf.FINAL_RESULT_PATH.format(
        "adult/" + mode + "/mobj-ninstance-{}-nsample-{}-size-{}-seed-{}.csv".format(n_instance, n_sample, emb_size,
                                                                                     seed)),
                             index=False)
